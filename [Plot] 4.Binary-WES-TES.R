...
plotter = function(TES1.VCF, panel, clonal, cols, order.by, out.file)
{
  ...
  pheatmap(TES1, 
           ...
           fontsize = 6,
           cellheight = 5,
           file = paste('READCOUNTS', panel, out.file, sep = '-'))
...
# x is larger than y -- extend y to x's rows
expander = function(x, y) {
  
  rows = unique(c(rownames(y), rownames(x)))
  cols = c(colnames(x), colnames(y))
  
  data = data.frame(matrix(NA, nrow = length(rows), ncol = length(cols)))  
  rownames(data) = rows
  colnames(data) = cols
  
  data[rownames(x), colnames(x)] = x
  data[rownames(y), colnames(y)] = y
  data
}
...
# rownames(a) = c('a', 'as')
# 
# sa = data.frame(matrix(c(1,2,3,4), nrow=2))
# rownames(sa) = c('a', 'afsaas')
# colnames(sa) = c('345453345', 'a54535s')
# 
# merge(a, sa)
...
# ######################################################################
setwd(CCF.FOLDER)
load('Patient_52_cancer_cell_fractions.csv.RData', verbose = T)
CCF[CCF > 0] = 1 # binarize that
colnames(CCF) = paste('CCF-', colnames(CCF), sep = '')
head(CCF)

setwd(WES.FOLDER)
load('NG-8132_52.mutect2.platypus_PASS.vcf.RData', verbose = T)
WES.VCF = WES.VCF$NV
WES.VCF[WES.VCF > 0] = 1 # binarize that
colnames(WES.VCF) = paste('WES-', colnames(WES.VCF), sep = '')
head(WES.VCF)

setwd(TES1.FOLDER)
load('patient_52_platypus.vcf.RData', verbose = T)
TES1.VCF = TES1.VCF$NV
colnames(TES1.VCF) = paste('TES1-', colnames(TES1.VCF), sep = '')
TES1.VCF[TES1.VCF > 0] = 1

setwd(TES2.FOLDER)
load('52.platypus.vcf.RData', verbose = T)
TES2.VCF = TES2.VCF$NV
TES2.VCF = TES2.VCF[rownames(TES2.VCF) %in% rownames(CCF), , drop = F]
colnames(TES2.VCF) = paste('TES2-', colnames(TES2.VCF), sep = '')
TES2.VCF[TES2.VCF > 0] = 1

data = expander(CCF, WES.VCF)
data = expander(data, TES1.VCF)
data = expander(data, TES2.VCF)
